/* Decompiler 2257ms, total 2762ms, lines 3202 */
...
import fun.kubik.events.main.misc.EventPush.PushType;
import fun.kubik.events.main.movement.EventJump;
import fun.kubik.events.main.player.EventDeath;
import fun.kubik.events.main.player.EventElytra;
import fun.kubik.events.main.player.EventItemPickup;
import fun.kubik.events.main.visual.EventHand.Speed;
...
public abstract class LivingEntity extends Entity {
    ...
    protected BrainCodec<?> getBrainCodec() {
    ...
    protected void updateFallState(double y, boolean onGroundIn, BlockState state, BlockPos pos) {
        ...
        if (!this.world.isRemote && this.fallDistance > 3.0F && onGroundIn) {
            ...
            if (!state.isAir()) {
                ...
                int i = (int)(150.0D * d0);
                ((ServerWorld)this.world).spawnParticle(new BlockParticleData(ParticleTypes.BLOCK, state), this.getPosX(), this.getPosY(), this.getPosZ(), i, 0.0D, 0.0D, 0.0D, 0.15000000596046448D);
            }
        }
    ...
    public void writeAdditional(CompoundNBT compound) {
        ...
        dataresult.resultOrPartial(var10001::error).ifPresent((p_233636_1_) -> {
    ...
    protected void dropLoot(DamageSource damageSourceIn, boolean attackedRecently) {
        ...
        Builder lootcontext$builder = this.getLootContextBuilder(attackedRecently, damageSourceIn);
    ...
    private int getArmSwingAnimationEnd() {
        Speed eventHand = new Speed(1.0F);
        EventManager.call(eventHand);
        if (eventHand.getSpeed() != 1.0F && this instanceof ClientPlayerEntity) {
            return (int)eventHand.getSpeed();
    ...
    protected void jump() {
        ...
        if (this instanceof ClientPlayerEntity) {
            EventJump event = new EventJump(f, this.rotationYaw);
            if (ClientPlayerEntity.class.isInstance(this)) {
                IItemics itemics = ItemicsAPI.getProvider().getItemicsForPlayer((ClientPlayerEntity)this);
                if (itemics != null) {
                    this.jumpRotationEvent = new RotationMoveEvent(Type.JUMP, this.rotationYaw);
                    itemics.getGameEventHandler().onPlayerRotationMove(this.jumpRotationEvent);
                    if (itemics.getMineProcess().isActive()) {
                        event = new EventJump(f, this.jumpRotationEvent.getYaw());
                    }
                }
            }
            ...
            Vector3d vector3d = this.getMotion();
            this.setMotion(vector3d.x, (double)event.getJumpMotion(), vector3d.z);
            if (this.isSprinting()) {
                float f1 = event.getYaw() * 0.017453292F;
    ...
    protected void handleFluidSneak() {
        this.setMotion(this.getMotion().add(0.0D, -0.03999999910593033D, 0.0D));
    }
    ...
    public void travel(Vector3d travelVector) {
        if (this.isServerWorld() || this.canPassengerSteer()) {
            ...
            if (this.isInWater() && this.func_241208_cS_() && !this.func_230285_a_(fluidstate.getFluid())) {
                ...
                this.setMotion(vector3d6.mul((double)f, 0.800000011920929D, (double)f));
                Vector3d vector3d2 = this.func_233626_a_(d0, flag, this.getMotion());
                this.setMotion(vector3d2);
                if (this.collidedHorizontally && this.isOffsetPositionInLiquid(vector3d2.x, vector3d2.y + 0.6000000238418579D - this.getPosY() + d8, vector3d2.z)) {
                    this.setMotion(vector3d2.x, 0.30000001192092896D, vector3d2.z);
                }
                ...
                if (this.func_233571_b_(FluidTags.LAVA) <= this.func_233579_cu_()) {
                    this.setMotion(this.getMotion().mul(0.5D, 0.800000011920929D, 0.5D));
                    vector3d4 = this.func_233626_a_(d0, flag, this.getMotion());
                ...
                if (this.collidedHorizontally && this.isOffsetPositionInLiquid(vector3d4.x, vector3d4.y + 0.6000000238418579D - this.getPosY() + d8, vector3d4.z)) {
                    this.setMotion(vector3d4.x, 0.30000001192092896D, vector3d4.z);
                }
                ...
                if (this instanceof ClientPlayerEntity) {
                    EventElytra eventElytra = new EventElytra(this.rotationPitch, this.rotationYaw);
                    EventManager.call(eventElytra);
                    vector3d1 = this.getVectorForRotation(eventElytra.getPitch(), eventElytra.getYaw());
                    f = eventElytra.getPitch() * 0.017453292F;
                }
                ...
                if (vector3d.y < 0.0D && d1 > 0.0D) {
                    d10 = vector3d.y * -0.1D * (double)f1;
                    vector3d = vector3d.add(vector3d1.x * d10 / d1, d10, vector3d1.z * d10 / d1);
                }

                if (f < 0.0F && d1 > 0.0D) {
                    d10 = d3 * (double)(-MathHelper.sin(f)) * 0.04D;
                    vector3d = vector3d.add(-vector3d1.x * d10 / d1, d10 * 3.2D, -vector3d1.z * d10 / d1);
                }
                ...
                if (this.collidedHorizontally && !this.world.isRemote) {
                    d10 = Math.sqrt(horizontalMag(this.getMotion()));
                    double d6 = d3 - d10;
                ...
                f = this.onGround ? f3 * 0.91F : 0.91F;
                ...
                this.setMotion(vector3d5.x * (double)f, d2 * 0.9800000190734863D, vector3d5.z * (double)f);
            }
        }
    ...
    private Map<EquipmentSlotType, ItemStack> func_241354_r_() {
        ...
        for(int var4 = 0; var4 < var3; ++var4) {
            EquipmentSlotType equipmentslottype = var2[var4];
            ItemStack itemstack;
    ...
    private ItemStack getItemInHand(EquipmentSlotType slot) {
        return (ItemStack)this.handInventory.get(slot.getIndex());
    }
    ...
    public void triggerItemPickupTrigger(ItemEntity item) {
        ...
        EventItemPickup eventItemPickup = new EventItemPickup(playerentity, item);
        EventManager.call(eventItemPickup);
    ...
    public boolean canEntityBeSeen(Entity entityIn) {
        ...
        return this.world.rayTraceBlocks(new RayTraceContext(vector3d, vector3d1, BlockMode.COLLIDER, FluidMode.NONE, this)).getType() == net.minecraft.util.math.RayTraceResult.Type.MISS;
    }
    ...
    public boolean canBePushed() {
        EventPush eventPush = new EventPush(PushType.Entities);
    ...
    private void addItemParticles(ItemStack stack, int count) {
        for(int i = 0; i < count; ++i) {
            ...
            vector3d = vector3d.rotatePitch(-this.rotationPitch * 0.017453292F);
            vector3d = vector3d.rotateYaw(-this.rotationYaw * 0.017453292F);
            double d0 = (double)(-this.rand.nextFloat()) * 0.6D - 0.3D;
            Vector3d vector3d1 = new Vector3d(((double)this.rand.nextFloat() - 0.5D) * 0.3D, d0, 0.6D);
            vector3d1 = vector3d1.rotatePitch(-this.rotationPitch * 0.017453292F);
            vector3d1 = vector3d1.rotateYaw(-this.rotationYaw * 0.017453292F);
    ...
    private boolean isInValidBed() {
        return (Boolean)this.getBedPosition().map((p_241350_1_) -> {
    ...
    static {
        SPRINTING_SPEED_BOOST = new AttributeModifier(SPRINTING_SPEED_BOOST_ID, "Sprinting speed boost", 0.30000001192092896D, Operation.MULTIPLY_TOTAL);
