/* Decompiler 43ms, total 439ms, lines 171 */
...
public class BipedArmorLayer<T extends LivingEntity, M extends BipedModel<T>, A extends BipedModel<T>> extends LayerRenderer<T, M> {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (customModel.isToggled()) {
            ...
            String entityName = entitylivingbaseIn.getName().getString();
            boolean self = customModel.getElements().getSelected("Self") && entitylivingbaseIn.getUniqueID().equals(client.getUniqueID());
            boolean friend = customModel.getElements().getSelected("Friends") && Load.getInstance().getHooks().getFriendManagers().is(entityName);
            boolean others = customModel.getElements().getSelected("Others") && !Load.getInstance().getHooks().getFriendManagers().is(entityName) && !entitylivingbaseIn.getUniqueID().equals(client.getUniqueID());
    ...
    private ResourceLocation func_241737_a_(ArmorItem p_241737_1_, boolean p_241737_2_, @Nullable String p_241737_3_) {
        String var10000 = p_241737_1_.getArmorMaterial().getName();
        String s = "textures/models/armor/" + var10000 + "_layer_" + (p_241737_2_ ? 2 : 1) + (p_241737_3_ == null ? "" : "_" + p_241737_3_) + ".png";
    ...
    protected A getArmorModelHook(T p_getArmorModelHook_1_, ItemStack p_getArmorModelHook_2_, EquipmentSlotType p_getArmorModelHook_3_, A p_getArmorModelHook_4_) {
        return Reflector.ForgeHooksClient_getArmorModel.exists() ?
                (A)Reflector.ForgeHooksClient_getArmorModel.call(p_getArmorModelHook_1_, p_getArmorModelHook_2_, p_getArmorModelHook_3_, p_getArmorModelHook_4_) :
                p_getArmorModelHook_4_;
    }
