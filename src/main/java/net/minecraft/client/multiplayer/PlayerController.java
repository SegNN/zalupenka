...
import fun.kubik.itemics.utils.accessor.IPlayerControllerMP;
import fun.kubik.modules.combat.Aura;
import fun.kubik.modules.player.NoInteract;
...
public class PlayerController
    ...
    public boolean onPlayerDestroyBlock(BlockPos pos) {
        ...
        boolean flag = ((World)world).setBlockState(pos, fluidstate.getBlockState(), 11);
        if (flag) {
    ...
    public ActionResultType processRightClickBlock(ClientPlayerEntity player, ClientWorld worldIn, Hand hand, BlockRayTraceResult blockRayTraceResult) {
        boolean flag2;
        NoInteract interactFunction = (NoInteract)Load.getInstance().getHooks().getModuleManagers().findClass(NoInteract.class);
        this.syncCurrentPlayItem();
        ...
        boolean bl = flag2 = interactFunction.isToggled() && (Boolean)interactFunction.allBlocks.getValue() != false || interactFunction.isToggled() && interactFunction.getBlocks().contains(worldIn.getBlockState(blockRayTraceResult.getPos()).getBlockId()) || ((Aura)Load.getInstance().getHooks().getModuleManagers().findClass(Aura.class)).getTarget() != null;
        if (!flag2) {
            ActionResultType actionresulttype;
            if (!flag1 && (actionresulttype = worldIn.getBlockState(blockpos).onBlockActivated(worldIn, player, hand, blockRayTraceResult)).isSuccessOrConsume()) {
                this.connection.sendPacket(new CPlayerTryUseItemOnBlockPacket(hand, blockRayTraceResult));
    ...
    public ActionResultType func_217292_a(ClientPlayerEntity p_217292_1_, ClientWorld p_217292_2_, Hand p_217292_3_, BlockRayTraceResult p_217292_4_) {
        ...
        boolean bl = flag1 = p_217292_1_.isSecondaryUseActive() && flag;
        if (!flag1 && (actionresulttype = p_217292_2_.getBlockState(blockpos).onBlockActivated(p_217292_2_, p_217292_1_, p_217292_3_, p_217292_4_)).isSuccessOrConsume()) {
            this.connection.sendPacket(new CPlayerTryUseItemOnBlockPacket(p_217292_3_, p_217292_4_));
            return actionresulttype;
        }
        this.connection.sendPacket(new CPlayerTryUseItemOnBlockPacket(p_217292_3_, p_217292_4_));
    ...
    public void windowClickFixed(int i, int slot, int button, ClickType clickType, ClientPlayerEntity player, int time) {
    }
