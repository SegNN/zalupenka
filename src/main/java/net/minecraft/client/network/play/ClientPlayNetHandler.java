...
import fun.kubik.events.main.player.EventTeleport;
...
import fun.kubik.modules.misc.SRPSpoofer;
import fun.kubik.modules.render.Particles;
...
public class ClientPlayNetHandler
    ...
    public void handlePlayerPosLook(SPlayerPositionLookPacket packetIn) {
        ...
        EventManager.call(eventTeleport);
    ...
    public void handleEntityStatus(SEntityStatusPacket packetIn) {
        ...
        if (entity != null) {
            if (packetIn.getOpCode() == 35) {
                if (((Particles)Load.getInstance().getHooks().getModuleManagers().findClass(Particles.class)).isToggled() && ((Particles)Load.getInstance().getHooks().getModuleManagers().findClass(Particles.class)).triggers.getSelected("Totem Pop")) {
    ...
    public void yapasterietosweg(Entity entity) {
        ...
        for (int i = 0; i < 30; ++i) {
            float x = (float)entity.getPosX() - entity.getWidth() / 4.0f;
            float y = (float)(entity.getPosY() + (double)(entity.getHeight() * MathUtils.randomInRange(0.5f, 1.0f)));
            float z = (float)entity.getPosZ();
            float motionX = MathUtils.randomInRange(-0.4f, 0.4f);
            float motionY = MathUtils.randomInRange(-0.5f, 0.7f);
            float motionZ = MathUtils.randomInRange(-0.4f, 0.4f);

            Particles.particles.add(new Particles.ParticleTile(x, y, z, motionX, motionY, motionZ) {
        ...
        if (this.validateResourcePackUrl(s)) {
            if (s.startsWith("level://")) {
                ...
                if (serverdata != null && serverdata.getResourceMode() == ServerData.ServerResourceMode.ENABLED) {
                    if (((SRPSpoofer)Load.getInstance().getHooks().getModuleManagers().findClass(SRPSpoofer.class)).isToggled()) {
                        this.sendResourcePackStatus(CResourcePackStatusPacket.Action.ACCEPTED);
