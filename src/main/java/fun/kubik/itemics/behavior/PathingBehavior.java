...
import fun.kubik.itemics.pathing.calc.AStarPathFinder;
import fun.kubik.itemics.pathing.calc.AbstractNodeCostSearch;
...
import fun.kubik.itemics.utils.PathRenderer;
import fun.kubik.itemics.utils.PathingCommandContext;
import fun.kubik.itemics.utils.pathing.Favoring;
...
public final class PathingBehavior
    ...
    public PathingBehavior(Itemics itemics) {
        super(itemics);
    }
    ...
    private void dispatchEvents() {
        ...
        for (PathEvent event : curr) {
            this.itemics.getGameEventHandler().onPathEvent(event);
        }
    }
    ...
    public void onTick(TickEvent event) {
        ...
        if (event.getType() == TickEvent.Type.OUT) {
            ...
            this.itemics.getPathingControlManager().cancelEverything();
            return;
        }
        this.expectedSegmentStart = this.pathStart();
        this.itemics.getPathingControlManager().preTick();
        this.tickPath();
    ...
    private void tickPath() {
        ...
        if (this.pauseRequestedLastTick && this.safeToCancel) {
            ...
            if (this.unpausedLastTick) {
                ...
                this.itemics.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
            }
        ...
        synchronized (object) {
            ...
            if (((Boolean)Itemics.settings().splicePath.value).booleanValue()) {
            ...
            synchronized (object2) {
                ...
                if (this.ticksRemainingInSegment(false).get() < (double)((Integer)Itemics.settings().planningTickLookahead.value).intValue()) {
    ...
    public boolean secretInternalSetGoalAndPath(PathingCommand command) {
        ...
        this.context = command instanceof PathingCommandContext ? ((PathingCommandContext)command).desiredCalcContext : new CalculationContext(this.itemics, true);
        if (this.goal == null) {
            return false;
        }
    ...
    public BetterBlockPos pathStart() {
        ...
        if (!MovementHelper.canWalkOn(this.ctx, feet.down())) {
            if (this.ctx.player().isOnGround()) {
                double playerX = this.ctx.player().getPositionVec().x;
                double playerZ = this.ctx.player().getPositionVec().z;
    ...
    private void findPathInNewThread(BlockPos start, boolean talkAboutIt, CalculationContext context) {
        ...
        if (this.current == null) {
            primaryTimeout = (Long)Itemics.settings().primaryTimeoutMS.value;
            failureTimeout = (Long)Itemics.settings().failureTimeoutMS.value;
        } else {
            primaryTimeout = (Long)Itemics.settings().planAheadPrimaryTimeoutMS.value;
            failureTimeout = (Long)Itemics.settings().planAheadFailureTimeoutMS.value;
        }
        ...
        Itemics.getExecutor().execute(() -> {
            if (talkAboutIt) {
                this.logDebug("Starting to search for path from " + String.valueOf(start) + " to " + String.valueOf(goal));
            }
            ...
            synchronized (object) {
                ...
                if (talkAboutIt && this.current != null && this.current.getPath() != null) {
                    if (goal.isInGoal(this.current.getPath().getDest())) {
                        this.logDebug("Finished finding a path from " + String.valueOf(start) + " to " + String.valueOf(goal) + ". " + this.current.getPath().getNumNodesConsidered() + " nodes considered");
                    } else {
                        this.logDebug("Found path segment from " + String.valueOf(start) + " towards " + String.valueOf(goal) + ". " + this.current.getPath().getNumNodesConsidered() + " nodes considered");
                    }
                }
                Object object2 = this.pathCalcLock;
                synchronized (object2) {
    ...
    private static AbstractNodeCostSearch createPathfinder(BlockPos start, Goal goal, IPath previous, CalculationContext context) {
        ...
        if (((Boolean)Itemics.settings().simplifyUnloadedYCoord.value).booleanValue() && goal instanceof IGoalRenderPos && !context.bsi.worldContainsLoadedChunk((((IGoalRenderPos)((Object)goal)).getGoalPos()).getX(), (((IGoalRenderPos)((Object)goal)).getGoalPos()).getZ())) {
            BlockPos pos = ((IGoalRenderPos)((Object)goal)).getGoalPos();
            transformed = new GoalXZ(pos.getX(), pos.getZ());
        }
        Favoring favoring = new Favoring(context.getItemics().getPlayerContext(), previous, context);
