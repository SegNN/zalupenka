...
public interface IVertexBuilder
    ...
    public static final ThreadLocal<RenderEnv> RENDER_ENV = ThreadLocal.withInitial(() -> new RenderEnv(Blocks.AIR.getDefaultState(), new BlockPos(0, 0, 0)));
    public static final boolean FORGE = Reflector.ForgeHooksClient.exists();

    default public RenderEnv getRenderEnv(BlockState p_getRenderEnv_1_, BlockPos p_getRenderEnv_2_) {
    ...
    default public IVertexBuilder color(int rgb) {
        float[] color = ColorHelpers.IntColor.rgb(rgb);
        return this.color(color[0], color[1], color[2], color[3]);
    }

    default public void addVertex(float x, float y, float z, float red, float green, float blue, float alpha, float texU, float texV, int overlayUV, int lightmapUV, float normalX, float normalY, float normalZ) {
        this.pos(x, y, z);
    ...
    default public IVertexBuilder color(float red, float green, float blue, float alpha) {
        return this.color((int)(red * 255.0f), (int)(green * 255.0f), (int)(blue * 255.0f), (int)(alpha * 255.0f));
    }

    default public IVertexBuilder lightmap(int lightmapUV) {
        return this.lightmap(lightmapUV & 0xFFFF, lightmapUV >> 16 & 0xFFFF);
    }

    default public IVertexBuilder overlay(int overlayUV) {
        return this.overlay(overlayUV & 0xFFFF, overlayUV >> 16 & 0xFFFF);
    }

    default public void addQuad(MatrixStack.Entry matrixEntryIn, BakedQuad quadIn, float redIn, float greenIn, float blueIn, int combinedLightIn, int combinedOverlayIn) {
    ...
    default public void addVertexData(MatrixStack.Entry p_addVertexData_1_, BakedQuad p_addVertexData_2_, float[] p_addVertexData_3_, float p_addVertexData_4_, float p_addVertexData_5_, float p_addVertexData_6_, float p_addVertexData_7_, int[] p_addVertexData_8_, int p_addVertexData_9_, boolean p_addVertexData_10_) {
    ...
    default public void addQuad(MatrixStack.Entry matrixEntryIn, BakedQuad quadIn, float[] colorMuls, float redIn, float greenIn, float blueIn, int[] combinedLightsIn, int combinedOverlayIn, boolean mulColor) {
        this.addQuad(matrixEntryIn, quadIn, colorMuls, redIn, greenIn, blueIn, 1.0f, combinedLightsIn, combinedOverlayIn, mulColor);
    }

    default public void addQuad(MatrixStack.Entry p_addQuad_1_, BakedQuad p_addQuad_2_, float[] p_addQuad_3_, float p_addQuad_4_, float p_addQuad_5_, float p_addQuad_6_, float p_addQuad_7_, int[] p_addQuad_8_, int p_addQuad_9_, boolean p_addQuad_10_) {
        ...
        for (int i1 = 0; i1 < k; ++i1) {
            ...
            if (FORGE && (vector3f = this.applyBakedNormals(aint, j1, p_addQuad_1_.getNormal())) != null) {
                f3 = vector3f.getX();
                f4 = vector3f.getY();
                f5 = vector3f.getZ();
            }
    ...
    default public IVertexBuilder pos(Matrix4f matrixIn, float x, float y, float z) {
    ...
    default public IVertexBuilder normal(Matrix3f matrixIn, float x, float y, float z) {
    ...
    default public void putSprite(TextureAtlasSprite p_putSprite_1_) {
    }

    default public void setSprite(TextureAtlasSprite p_setSprite_1_) {
    }
    ...
    default public void setRenderType(RenderType p_setRenderType_1_) {
    }
    ...
    default public Vector3f getTempVec3f(Vector3f p_getTempVec3f_1_) {
        return p_getTempVec3f_1_.copy();
    }

    default public Vector3f getTempVec3f(float p_getTempVec3f_1_, float p_getTempVec3f_2_, float p_getTempVec3f_3_) {
    ...
    default public int applyBakedLighting(int p_applyBakedLighting_1_, int[] p_applyBakedLighting_2_, int p_applyBakedLighting_3_) {
        int i = IVertexBuilder.getLightOffset(0);
    ...
    public static int getLightOffset(int p_getLightOffset_0_) {
        return p_getLightOffset_0_ * 8 + 6;
    }

    default public Vector3f applyBakedNormals(int[] p_applyBakedNormals_1_, int p_applyBakedNormals_2_, Matrix3f p_applyBakedNormals_3_) {
        int i = 7;
